...
class Index
{
    ...
    public function saveSetting()
    {
        ...
        $newPass = input("pass");
        if ($newPass != "******") {
            Db::name("setting")->where("vkey", "pass")->update(array("vvalue" => $newPass));
        }

        // 对输入进行过滤
        Db::name("setting")->where("vkey", "notifyUrl")->update(array("vvalue" => filter_var(input("notifyUrl"), FILTER_SANITIZE_URL)));
        Db::name("setting")->where("vkey", "returnUrl")->update(array("vvalue" => filter_var(input("returnUrl"), FILTER_SANITIZE_URL)));
        Db::name("setting")->where("vkey", "key")->update(array("vvalue" => input("key")));
        Db::name("setting")->where("vkey", "close")->update(array("vvalue" => intval(input("close"))));
        Db::name("setting")->where("vkey", "payQf")->update(array("vvalue" => intval(input("payQf"))));
        Db::name("setting")->where("vkey", "wxpay")->update(array("vvalue" => filter_var(input("wxpay"), FILTER_SANITIZE_URL)));
        Db::name("setting")->where("vkey", "zfbpay")->update(array("vvalue" => filter_var(input("zfbpay"), FILTER_SANITIZE_URL)));
    ...
    public function delAllOrder()
    {
        ...
        Db::name("pay_order")->where('1=1')->delete();
        Db::name("tmp_price")->where('1=1')->delete();
        return json($this->getReturn());
    }
